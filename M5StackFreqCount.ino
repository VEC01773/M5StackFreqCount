...
//
volatile int16_t PulsCounter;       //パルスカウント
volatile int16_t PrePulsCounter;    //前回読み込みしたパルスカウント
...
void loop()
{
    int countdiff = PulsCounter - PrePulsCounter;
    uint32_t timediff;
    if (countdiff < 0)
        countdiff += 0x7fff;
    ...
    Serial.printf("frequency :%0.3fKHz %d %d %u %u %u\n", f, PulsCounter, countdiff, CurTime, PreTime, timediff);
    //  M5.Lcd.clear();
